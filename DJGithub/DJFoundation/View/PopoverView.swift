...
class PopoverView: UIView {
  ...
  func showPopover(in rect: CGRect, sourceRect: CGRect) {
    ...
    switch direction {
    case .down:
      point = CGPoint(x: x, y: y - contentView.bounds.height / 2 - CGFloat(interval) - sourceViewInWindowFrame.size.height / 2)
    default:
      break
    }
    ...
    createPopoverLayer(triangleOrigin: CGPoint(x: point.x - contentView.frame.origin.x - triangleSize.width / 2, y: contentView.frame.size.height), triangleSize: triangleSize)
  }
  ...
  func createPopoverLayer(triangleOrigin: CGPoint, triangleSize: CGSize) {
    ...
    path.addArc(withCenter: CGPoint(x: contentView.frame.size.width - cornerRadius, y: cornerRadius), radius: cornerRadius, startAngle: -.pi / 2, endAngle: 0, clockwise: true)
    if (triangleOrigin.x + triangleSize.width) >= (contentView.frame.origin.x + contentView.frame.size.width - cornerRadius) {
      path.addLine(to: CGPoint(x: contentView.frame.size.width, y: contentView.frame.size.height))
      path.addLine(to: CGPoint(x: contentView.frame.size.width - triangleSize.width / 2, y: contentView.frame.size.height + triangleSize.height))
      ...
      path.addArc(withCenter: CGPoint(x: contentView.frame.size.width - cornerRadius, y: contentView.frame.size.height - cornerRadius), radius: cornerRadius, startAngle: 0, endAngle: .pi / 2, clockwise: true)
      path.addLine(to: CGPoint(x: triangleOrigin.x + triangleSize.width, y: contentView.frame.height))
      path.addLine(to: CGPoint(x: triangleOrigin.x + triangleSize.width / 2, y: contentView.frame.height + triangleSize.height))
    ...
    path.addArc(withCenter: CGPoint(x: cornerRadius, y: contentView.frame.size.height - cornerRadius), radius: cornerRadius, startAngle: .pi / 2, endAngle: .pi, clockwise: true)
