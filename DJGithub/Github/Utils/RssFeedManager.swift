...
class RssFeedManager: NSObject {
  ...
  var loadedReadMappingCountClosure: (() -> Void)?
  var finishedLoadFeeds: (([RssFeedLatestCellModel]) -> Void)?
  ...
  func loadReadMapping(_ completionHandler: @escaping ([Int: (Int, Int)]) -> Void) {
  ...
  func loadLatestFeeds(by limit: Int = 5, completionHandler: @escaping ([RssFeedLatestCellModel]) -> Void) {
    DispatchQueue.global(qos: .utility).async {
      ...
      let models = feeds.compactMap { feed in
        if let atom = self.atomMapping[feed.atomId] {
            let date = feed.updated.components(separatedBy: " ").first ?? feed.updated
