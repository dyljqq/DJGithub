...
class RepoIssueDetailHeaderView: UIView {
  ...
  var touchedTitleClosure: (() -> Void)?
  var updateStateClosure:(() -> Void)?
  ...
  func render(with issue: IssueDetail, comletionHandler: ((CGFloat) -> Void)? = nil) {
    self.issue = issue
    ...
    let titleHeight = (titleLabel.text! as NSString).boundingRect(
...
class RepoIssueDetailViewController: UIViewController {
  ...
  private func setUp() {
    ...
    headerView.updateStateClosure = { [weak self] in
      ...
      alertVC.addAction(UIAlertAction(title: "sure", style: .default, handler: { _ in
        Task {
